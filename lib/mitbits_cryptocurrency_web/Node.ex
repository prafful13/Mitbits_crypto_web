defmodule MitbitsCryptocurrencyWeb.Node do
  use GenServer, restart: :transient

  # API
  def start_link({pk, sk, genesis_block, hash_name}) do
    GenServer.start_link(__MODULE__, {pk, sk, genesis_block},
      name: MitbitsCryptocurrencyWeb.Utility.string_to_atom("node_" <> hash_name)
    )
  end

  def get_balance(hash) do
    GenServer.call(MitbitsCryptocurrencyWeb.Utility.string_to_atom("node_" <> hash), :get_balance)
  end
          ...
          if(Kernel.is_map(txn.message) == true) do
            if(txn.message.from =~ "miner") do
              {_, map} =
                Map.get_and_update(
                  map,
                  MitbitsCryptocurrencyWeb.Utility.string_to_atom(txn.message.to),
                  fn current_value ->
...
#              )
  ...
  def handle_call(
    ...
    my_name = "node_" <> MitbitsCryptocurrencyWeb.Utility.getHash(pk)
  ...
  def handle_call(
    ...
    GenServer.cast(
      MitbitsCryptocurrencyWeb.Utility.string_to_atom("node_" <> miner_node_hash),
      {:req_for_MitbitsCryptocurrencyWeb, 10, MitbitsCryptocurrencyWeb.Utility.getHash(pk)}
    )
