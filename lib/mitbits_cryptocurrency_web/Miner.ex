defmodule MitbitsCryptocurrencyWeb.Miner do
  ...
  def start_link({pk, sk, hash_name}) do
    GenServer.start_link(__MODULE__, {pk, sk},
      name: MitbitsCryptocurrencyWeb.Utility.string_to_atom("miner_" <> hash_name)
    )
  end

  def start_mining(hash) do
    GenServer.cast(MitbitsCryptocurrencyWeb.Utility.string_to_atom("miner_" <> hash), :mine)
  end
  ...
  def handle_call({:mine_first, string}, _from, {pk, sk}) do
    signature = MitbitsCryptocurrencyWeb.Utility.sign(string, sk)
  ...
  def handle_cast(:mine, {pk, sk}) do
    ...
    authenticated_txn_list =
      Enum.reduce(curr_unchained_txns, [], fn txn, temp_list ->
        ...
        from_hash = MitbitsCryptocurrencyWeb.Utility.string_to_atom(txn.message.from)
        balance_from_hash = Map.get(indexed_blockchain, from_hash)

        if MitbitsCryptocurrencyWeb.Utility.verify(string, signature, from_pk) == true &&
    ...
    if Enum.count(sorted_unchained_txns) < size_of_txn_set do
      ...
      signature_of_reward_txn = MitbitsCryptocurrencyWeb.Utility.sign(str_reward_msg, sk)
      ...
      str_txn_set = MitbitsCryptocurrencyWeb.Utility.combine(txn_set)

      [{_, prev_block_hash}] = :ets.lookup(:MitbitsCryptocurrencyWeb, "prev_block_hash")
...
#        )
  ...
  def find_block_hash(string, nonce, prev_block_hash, my_hash) do
    ...
    [{_, temp_prev_block_hash}] = :ets.lookup(:MitbitsCryptocurrencyWeb, "prev_block_hash")
    # IO.puts nonce
